...
public class SmithingData {
    ...
    public void init() {
        file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.SMITHING_SETTINGS_FILE_NAME);

        if (!file.exists()) {
            OPKingdomsCore.getInstance().saveResource(JobsString.SMITHING_SETTINGS_FILE_NAME, false);
            file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.SMITHING_SETTINGS_FILE_NAME);
        }
