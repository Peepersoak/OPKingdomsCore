...
public class ArcherData {
    ...
    public void init() {
        file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.ARCHER_SETTINGS_FILE_NAME);

        if (!file.exists()) {
            OPKingdomsCore.getInstance().saveResource(JobsString.ARCHER_SETTINGS_FILE_NAME, false);
            file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.ARCHER_SETTINGS_FILE_NAME);
        }
