...
public class EnchanterData {
    ...
    public void init() {
        file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.ENCHANTER_SETTINGS_FILE_NAME);

        if (!file.exists()) {
            OPKingdomsCore.getInstance().saveResource(JobsString.ENCHANTER_SETTINGS_FILE_NAME, false);
            file = new File(OPKingdomsCore.getInstance().getDataFolder(), JobsString.ENCHANTER_SETTINGS_FILE_NAME);
        }
